        ...
        for day in self.day_list:
            for hour in range(0,23):
                for driver in self.drivers:
                    if driver.schedule[day] == hour:
                        #Give random variable for arrival
                        #call speed randomizer from driver
                        driver.next_driver = self.head



    
class Driver:
    def __init__(self, ID, Attendance_average, speedmin, speedmax, varmin, varmax, next_driver=None):
        self.ID = ID
